...
import { useReserveMeeingStore } from '@/stores/reserveMeeting'
import { useRouter } from 'vue-router'
import { ref } from 'vue'
const router = useRouter()
const MyMeetingStore = useReserveMeeingStore()
...
<template>
  <div class="w-full flex flex-col items-center">
    <div class="w-11/12">
      <div class="font-bold text-lg mt-4 mb-4">组织日程</div>
      <div class="flex flex-col gap-3">
        <div v-for="item in MyMeetingStore.orgnizeMeeting" class="gap-4 flex">
          ...
          <div style="border-bottom: 1px solid #ccc" class="h-20 w-full">
            <div class="flex gap-5">
              ...
              <el-tag :color="item.type" effect="dark" class="ml-auto text-sm">{{
                item.signStatus
              }}</el-tag>
            </div>
            ...
            <div class="text-cyan-600 flex" @click="handle">
              <el-icon><MoreFilled /></el-icon>
              点击查看详情
              <el-tag class="ml-auto" type="warning" effect="dark">需要签到</el-tag>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="w-11/12">
      <div class="font-bold text-lg mt-4 mb-4">参会日程</div>
      <div class="flex flex-col gap-3">
        <div v-for="item in MyMeetingStore.joinMeeing" class="gap-4 flex">
          <img :src="item.url" alt="" class="h-20 w-28" />
          <div style="border-bottom: 1px solid #ccc" class="h-20 w-full">
            <div class="flex gap-5">
              <div class="font-semibold">{{ item.theme }}</div>
  ...
  <el-drawer v-model="isShow" :direction="direction" :with-header="false">
    <div class="flex flex-col gap-4">
      <div class="text-xl">
        {{ MyMeetingStore.orgnizeMeeting[0].theme }}
      </div>
      <div class="flex gap-4 items-center">
        <el-icon :size="20"><DeleteLocation /></el-icon>
        <span>{{ MyMeetingStore.orgnizeMeeting[0].location }}</span>
      </div>
      <div class="flex items-center gap-4">
        <el-icon :size="20"><Timer /></el-icon>
        <span>{{ MyMeetingStore.orgnizeMeeting[0].time }}</span>
      </div>
      <div class="flex gap-4 items-center">
      ...
      <div class="flex gap-4">
        ...
        <div class="flex-1">
          <div>内容介绍</div>
          <el-input v-model="textarea" type="textarea" autosize></el-input>
        </div>
      </div>
      ...
      <div class="flex gap-4 items-center">
        ...
        <el-tag effect="dark">需要签到</el-tag>
      </div>
    </div>
  </el-drawer>
</template>
